...
Если у вас нет возможности открыть порты, вы можете использовать ВПН `RadminVPN` или `ZeroTier`, а так же прокси ``Playit.gg`` (оффициально не поддерживается)
...
### Хост с использованием стороннего сервиса playit.gg
⚠️ **Этот метод не поддерживается участниками Fika, тех.поддержка не предоставляется** ⚠️
